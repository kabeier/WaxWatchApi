...
- Made `get_or_create_preferences` race-safe under concurrent first access by wrapping create/flush in a savepoint, retrying lookup on unique conflicts, and adding a concurrent-creation regression test to verify a single persisted preference row with no uncaught exception.
